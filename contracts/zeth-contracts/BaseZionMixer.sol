...
// zkSNARK is used)
contract BaseZionMixer is MerkleTreeMiMC7, ERC223ReceivingContract {
    ...
    // of the mixer contract accordingly. (ie: Appends the commitments to the
    // tree, appends the nullifiers to the list and so on).
    function check_mkroot_nullifiers_hsig(
        uint256[4] memory vk,
        uint256[nbInputs] memory primary_inputs)
    internal {
        // 1. We re-assemble the full root digest and check it is in the tree
    ...
    function assemble_and_return_commitments(
        uint256[nbInputs] memory primary_inputs)
    internal returns (bytes32[jsOut] memory) {
        ...
        for (uint256 i = 0; i < jsOut; i++) {
            ...
            commitments[i] = current_commitment;
        }
        return commitments;
    }
